...
async function main() {
  ...
  const tx1 = await wallet.writeContract({
    address: TOKEN, abi, functionName: "transfer", args: [you, parseUnits("100", 18)],
  ...
  const tx2 = await wallet.writeContract({
    address: TOKEN, abi, functionName: "approve", args: [you, parseUnits("50", 18)],
  ...
  console.log("allowance:", formatUnits(alw, 18), "DLAB");
